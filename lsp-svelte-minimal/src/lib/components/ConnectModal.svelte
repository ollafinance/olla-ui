<script lang="ts">
    ...
    const ICONS: Record<string, string> = {
        ...
        'coinbaseWallet': '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>'
    };
    ...
    function getConnectorIcon(connector: any) {
        if (connector.name.toLowerCase().includes('metamask')) return ICONS['metaMask'];
        if (connector.id === 'safe') return ICONS['safe'];
        if (connector.id === 'walletConnect') return ICONS['walletConnect'];
        if (connector.id === 'coinbaseWallet') return ICONS['coinbaseWallet'];
    ...
    >
        ...
        >
            ...
            <div class="px-6 pt-6 pb-2 flex justify-between items-center">
                ...
                <button 
                    onclick={() => wallet.closeModal()}
                    class="bg-gray-100 hover:bg-gray-200 text-gray-500 hover:text-gray-700 transition-all p-2 rounded-full"
                    aria-label="Close"
                >
            ...
            <div class="p-4 space-y-2">
                ...
                {#each wallet.connectors as connector}
                    <button
                        ...
                        class="w-full flex items-center justify-between p-4 rounded-xl bg-gray-50 hover:bg-gray-100 active:scale-[0.98] transition-all group text-left border border-transparent hover:border-gray-200"
                    >
                        <span class="font-bold text-gray-800 text-base">
                            {LABELS[connector.id] || connector.name}
                        </span>
                        
                        <div class="w-10 h-10 rounded-xl bg-white shadow-sm border border-gray-100 p-1.5 text-gray-600 group-hover:scale-110 transition-transform">
                ...
                {#if wallet.connectors.length === 0}
                    <div class="text-center py-12 text-gray-400">
                        ...
                        <p class="text-xs mt-1">Try installing MetaMask</p>
                    </div>
                {/if}
            </div>
