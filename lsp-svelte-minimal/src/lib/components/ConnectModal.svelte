<script lang="ts">
    ...
    const ICONS: Record<string, string> = {
        metaMask:
            '<svg viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg"><path d="M27.425 2.854l-5.467 5.619 2.508-8.473zM4.775 2.854l2.959-2.854 2.508 8.473zM14.775 24.437l1.092 3.693 1.092-3.693 7.85-4.482-1.425-9.675-7.517 3.596-7.517-3.596-1.425 9.675zM8.883 17.587l2.825 8.921-6.933-4.325zM20.292 26.508l2.825-8.921 4.108 4.596zM28.025 8.783l2.85 4.308-5.325 3.167 1.5-6.683zM4.175 8.783l0.975 0.792 1.5 6.683-5.325-3.167z" fill="#E17726"/></svg>',
        phantom:
            '<svg viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg"><path d="M27.065 4.933c-3.266-3.236-8.567-3.236-11.833 0l-1.166 1.155-1.166-1.155c-3.266-3.236-8.567-3.236-11.833 0-3.411 3.407-3.411 8.932 0 12.339l13 12.875 13-12.875c3.411-3.407 3.411-8.932 0-12.339zm-4.733 9.479c-1.633 0-2.966-1.321-2.966-2.943s1.333-2.943 2.966-2.943 2.966 1.321 2.966 2.943-1.333 2.943-2.966 2.943zm-12.666 0c-1.633 0-2.966-1.321-2.966-2.943s1.333-2.943 2.966-2.943 2.966 1.321 2.966 2.943-1.333 2.943-2.966 2.943z" fill="#AB9FF2"/></svg>',
        injected:
        ...
        walletConnect:
            '<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7 10l5-3 5 3M7 10v4l5 3 5-3v-4" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
        coinbaseWallet:
            '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 15h-2v-2h2v2zm0-4h-2V7h2v2z"/></svg>',
    };
    ...
    function getConnectorIcon(connector: any) {
        if (connector.icon)
            return `<img src="${connector.icon}" alt="${connector.name}" class="w-full h-full object-contain" />`;
    ...
    >
        ...
        >
            ...
            <div class="p-4 space-y-2">
                {#if wallet.error}
                    <div
                        class="mb-4 p-3 bg-red-50 text-red-600 text-xs font-medium rounded-xl border border-red-100 flex gap-2 items-start"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                ...
                <div class="flex flex-col gap-2">
                    {#each wallet.connectors as connector (connector.uid || connector.id)}
                        ...
                        >
                            ...
                            <div
                                class="w-10 h-10 rounded-xl bg-white shadow-sm border border-gray-100 p-1.5 text-gray-600 group-hover:scale-110 transition-transform flex items-center justify-center"
                            >
                ...
                {#if wallet.connectors.length === 0}
                    <div class="text-center py-12 text-gray-400">
                        ...
                        <p class="text-xs mt-1">
                            Try installing MetaMask or similar.
                        </p>
                    </div>
                {/if}
            </div>
            ...
            <!-- Footer -->
            <div
                class="px-6 py-4 bg-gray-50 border-t border-gray-100 text-center"
            >
                <p
